...
screen_size = 700
screen = pygame.display.set_mode((screen_size,screen_size)) # Creates window with width of 800px and height of 400px
...
class Circle(pygame.sprite.Sprite):
    ...
    def show_type(self):
        ...
        self.image = pygame.transform.rotozoom(self.image,0,screen_size/1000).convert_alpha()
    
    def deselect_selected_circle(self, move=True):
        selected_circle = [circle for circle in Circle.all if circle.id == Circle.selected_circle][0]
        if move:
    ...
    def toggle_type(self, click_type):
        ...
        move_pin_condition = (self.type == 'empty') and (Circle.selected_circle != None) # to move a pin, user must select an empty space and a selected circle/pin must exist
        ...
        if (placing_phase or move_pin_condition) and self.type == 'empty':
            ...
            if last_move in ['red', None]:
                ...
                self.type = 'red'
                last_move = 'red'
            if move_pin_condition: self.deselect_selected_circle() # Removes gold ring from previously selected pin and moves it
        elif self.selected:
            ...
            self.selected=True
            if Circle.selected_circle != None: self.deselect_selected_circle(False) # Removes previously selection without moving pin
...
def computer_move_pin():
    ...
    possible_destinations = {id:find_empty_neighbours(id) for id in computer_pins if len(find_empty_neighbours(id)) > 0}
...
while True: # infinite loop to stop screen from immediately closing
    ...
    for event in pygame.event.get():
        # INPUTS
        if event.type == pygame.QUIT: # all quit actions (e.g. closing the window) will stop the program
    ...
    if page == 'menu': # MENU PAGE ------------------------------------------------------------------------------------------------------
        ...
        back_button.draw()
        if back_button.check_click(): page = 'menu'
        ...
        for line in [('north', 'south'), ('east', 'west'), ('northeast', 'southwest'), ('northwest', 'southeast')]:
            pygame.draw.line(screen,line_details[0],board_coordinates[line[0]],board_coordinates[line[1]],width=line_details[1])
        pygame.draw.circle(screen,line_details[0],board_coordinates['centre'],screen_size*(2/6), width=line_details[1])
        board.draw(screen)
        for circle in [circle for circle in board if circle.selected == True]:
            pygame.draw.circle(screen,colours['gold'],circle.rect.center,circle.rect.size[0]/2,width=10)
        ...
        if check_illegal_condition():
            ...
            game_won = True
            write(f'{find_winner().capitalize()} Wins!', (screen_size/2,100), size=65, pixel_font=True) #type: ignore
        ...
        clear_button.draw()
        if clear_button.check_click(): clear_game()
        ...
        if page == 'single_player':
            ...
            if last_move == None: last_move = 'red'
            whose_turn_display = {
                'blue': (600,670),
                'red': (75,670)
            }
